...
const Events = () => {
  ...
  const saveEvent = (startDate, title, description) => {
    ...
    axios.post("http://localhost:4000/api/events", eventObj).then((res) => {
      console.log(res);
    });
  };
  const deleteEvent = () => {
    const eventToDelete = currentEvent.eventId;
    axios.delete(`/api/events/${eventToDelete}`).then((res) => {
      console.log(res);
    });
  ...
  const updateEvent = () => {
    ...
    return axios
      .put(
        `/api/events/?userId=${user}&eventId=${currentEvent.eventId}`,
        currentEvent
      )
      .then((res) => {
      ...
      >
        ...
        <Modal.Body>
          <Form>
            ...
            <Form.Group
              className="mb-3"
              controlId="eventDetailsForm.description"
            >
